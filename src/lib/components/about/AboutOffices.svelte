			...
			{#each offices as office, i}
				<div class="office-card">
					<TiltCard tiltAmount={8} glareOpacity={0.1}>
						...
						>
							<div
								class="w-full h-32 rounded-xl bg-gradient-to-br from-white/[0.03] to-white/[0.01] border border-line mb-6 flex items-center justify-center overflow-hidden relative"
							>
								<MapPin
									size={28}
									class="text-fg/10 group-hover:text-sky-500/30 transition-colors duration-500"
								/>
								<div
							...
							<h3
								class="text-xl font-black font-display text-fg mb-3 group-hover:text-sky-300 transition-colors"
							>
								{office.city}
							</h3>
							...
							<div class="space-y-3">
								<div class="flex items-start gap-3">
									<MapPin size={14} class="text-fg/20 mt-0.5 flex-shrink-0" />
									<p class="text-fg/40 text-sm leading-relaxed">{office.address}</p>
								</div>
								<div class="flex items-center gap-3">
									<Phone size={14} class="text-fg/20 flex-shrink-0" />
									<p class="text-fg/40 text-sm">{office.phone}</p>
								</div>
							</div>
						</div>
					</TiltCard>
