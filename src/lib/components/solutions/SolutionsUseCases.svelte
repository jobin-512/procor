...
<section class="relative py-24 md:py-32 px-6 md:px-12 bg-bg overflow-hidden">
	...
	<div class="max-w-7xl mx-auto relative z-10">
		<div class="text-center mb-16">
			...
			/>
			<p class="text-fg/60 text-lg max-w-xl mx-auto">From recruitment to retirement, PROCOR covers every HR workflow you need.</p>
		</div>
			...
			{#each useCases as uc, i}
				<TiltCard tiltAmount={6} glareOpacity={0.12} className="use-case-card">
					<GlowCard className="p-7 h-full group cursor-pointer" glowColor="rgba(117, 194, 246, 0.15)">
						<div class="w-12 h-12 rounded-xl bg-gradient-to-br from-sky-500/20 to-indigo-600/20 flex items-center justify-center border border-line mb-5 group-hover:scale-110 transition-transform duration-300">
							<svelte:component this={uc.icon} size={24} class="text-sky-400" />
						</div>
						<h3 class="text-fg font-bold text-lg mb-2 font-display">{uc.title}</h3>
						<p class="text-fg/50 text-sm leading-relaxed mb-5">{uc.desc}</p>
