...
<div bind:this={pageEl} class="min-h-screen bg-bg selection:bg-emerald-500/30 overflow-hidden">
	<!-- WebGL Background -->
	...
	<section class="relative min-h-screen flex items-center justify-center pt-32 pb-20 px-6 md:px-12">
		<div class="absolute inset-0 bg-gradient-to-b from-transparent via-bg/50 to-bg z-[1]"></div>
		...
		<div class="hero-content max-w-5xl mx-auto text-center relative z-10">
			...
			<h1 class="text-5xl sm:text-6xl md:text-7xl lg:text-8xl font-black text-fg leading-[0.9] font-display mb-8">
			...
			<div class="grid grid-cols-2 md:grid-cols-4 gap-6">
				{#each stats as stat}
					<div class="p-6 rounded-2xl bg-surface-2 border border-line backdrop-blur-xl">
						<svelte:component this={stat.icon} size={24} class="text-emerald-400 mb-3" />
						<div class="text-3xl font-black text-fg mb-1">{stat.value}</div>
						<div class="text-sm text-fg/50">{stat.label}</div>
					</div>
				{/each}
			</div>
		</div>
			...
			<div class="reveal-section flex flex-wrap justify-center gap-3 mb-12">
				{#each industries as industry, i}
					<button
						...
						class="group flex items-center gap-3 px-6 py-4 rounded-xl font-semibold transition-all duration-300 {activeIndustry === i ? 'bg-gradient-to-r ' + industry.gradient + ' text-fg shadow-lg scale-105' : 'bg-surface-2 text-fg/60 hover:bg-surface-2 hover:text-fg'}"
					>
			...
			<div class="reveal-section grid gap-12">
				...
				<div>
					<div class="w-16 h-16 rounded-2xl bg-gradient-to-br {industries[activeIndustry].gradient} flex items-center justify-center mb-6 shadow-lg">
						<svelte:component this={industries[activeIndustry].icon} size={32} class="text-fg" />
					</div>
					<h3 class="text-3xl md:text-4xl font-black text-fg mb-4">{industries[activeIndustry].name}</h3>
					<p class="text-xl text-fg/60 mb-8">{industries[activeIndustry].description}</p>

					<!-- Challenges & Benefits -->
					<div class="grid sm:grid-cols-2 gap-6 mb-8">
						<div>
							<h4 class="text-sm font-bold text-fg/40 uppercase tracking-wider mb-4">Challenges We Solve</h4>
							{#each industries[activeIndustry].challenges as challenge}
								<div class="flex items-center gap-3 mb-3">
									<div class="w-6 h-6 rounded bg-surface-2 flex items-center justify-center">
										<Zap size={14} class="text-amber-400" />
									</div>
									<span class="text-fg/70">{challenge}</span>
								</div>
							{/each}
						</div>
						<div>
							<h4 class="text-sm font-bold text-fg/40 uppercase tracking-wider mb-4">Key Benefits</h4>
							{#each industries[activeIndustry].benefits as benefit}
								<div class="flex items-center gap-3 mb-3">
									...
									<span class="text-fg/70">{benefit}</span>
								</div>
							{/each}
						</div>
					</div>

					<a href="/demo" class="inline-flex items-center gap-2 px-8 py-4 bg-gradient-to-r {industries[activeIndustry].gradient} text-fg rounded-xl font-semibold hover:shadow-lg transition-all group">
		...
		<div class="max-w-6xl mx-auto relative z-10">
			...
			<div class="reveal-section grid md:grid-cols-2 lg:grid-cols-4 gap-6">
				{#each companySizes as size, i}
					<div class="group p-6 rounded-2xl bg-surface-2 border border-line hover:bg-surface-2 hover:border-blue-500/30 transition-all duration-500">
						<div class="text-3xl font-black text-fg mb-2">{size.range}</div>
						<div class="text-blue-400 font-semibold mb-3">{size.name}</div>
						<p class="text-fg/50 text-sm mb-6">{size.description}</p>
		...
		<div class="max-w-4xl mx-auto">
			...
			<div class="reveal-section">
				<div class="relative">
					...
					<div class="relative p-8 md:p-12 rounded-3xl bg-surface-2 border border-line backdrop-blur-xl">
						<div class="mb-8">
							<label class="block text-fg font-semibold mb-4">Number of Employees</label>
							<div class="flex items-center gap-6">
								<input
									...
									max="10000"
									step="50"
									class="flex-1 h-2 bg-surface-2 rounded-full appearance-none cursor-pointer accent-amber-500"
								/>
								<div class="text-3xl font-black text-fg min-w-[100px] text-right">
						...
						{#if roiResult}
							<div class="mt-8 pt-8 border-t border-line">
								<div class="grid md:grid-cols-3 gap-6 text-center">
									<div>
										<div class="text-sm text-fg/50 mb-2">Current HR Cost (Est.)</div>
										<div class="text-2xl font-bold text-fg">${roiResult.currentCost.toLocaleString()}/yr</div>
									</div>
									<div>
										<div class="text-sm text-fg/50 mb-2">PROCOR Cost</div>
										<div class="text-2xl font-bold text-fg">${roiResult.procorCost.toLocaleString()}/yr</div>
									</div>
									<div>
										<div class="text-sm text-fg/50 mb-2">Net Savings</div>
	...
	<section class="relative py-32 px-6 md:px-12">
		...
		<div class="reveal-section max-w-4xl mx-auto text-center relative z-10">
			...
			<div class="flex flex-col sm:flex-row gap-4 justify-center">
				...
				<a href="/contact" class="px-10 py-5 bg-surface-2 border border-line text-fg rounded-2xl font-semibold text-lg hover:bg-surface-2 transition-all">
					Talk to an Expert
				</a>
			</div>
		</div>
